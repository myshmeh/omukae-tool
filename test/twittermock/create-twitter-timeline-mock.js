...
(async () => {
  ...
  await page.evaluate(() => {
    document.querySelectorAll('article[role="article"]')[0].click();
  });
  ...
  const result = await page.evaluate(() => {
    document
      .querySelector('div[aria-label="Back"]')
      .setAttribute(
        "onClick",
        "location='http://localhost:8080/notifications'"
      );
    return {
  ...
  fs.writeFile(
    "test/mockpages/twitter-timeline.html",
